 ...
 * Date: Sun Apr 28 2013 12:01:46 GMT+0800 (CST)
 *
...
Ext.define('HEART.controller.EMXsController', {
	...
	onEMAInitialize: function(component, eOpts) {
		...
		for(i=0; i< items.length; i++){
			...
			onChange = function() {
				...
				next.show();
				setTimeout(function(){component.parent.getScrollable().getScroller().scrollToEnd({type: "slide"});}, 128);
			};
	...
	onAudioPanelInitialize: function(component, eOpts) {
		...
		if(Math.random()<HEART.probability){
			...
			preSlider.on( { change: function() { audioButton.show(); setTimeout(function(){component.parent.getScrollable().getScroller().scrollToEnd({type: "slide"});}, 128);} } );
			audioButton.hide();
			postSlider.setShowAnimation({type: "fadeIn", duration: 1024});
			postSlider.hide();
			postSlider.on( { change: function() { button.show(); setTimeout(function(){component.parent.getScrollable().getScroller().scrollToEnd({type: "slide"});}, 128);} } );
			HEART.audioCallback =function() {
				postSlider.show();
				setTimeout(function(){component.parent.getScrollable().getScroller().scrollToEnd({type: "slide"});}, 128);
			}
	...
	onReflectionInitialize: function(component, eOpts) {
		...
		write.on( { tap: function() { write.hide(); reflect.show(); setTimeout(function(){component.parent.getScrollable().getScroller().scrollToEnd({type: "slide"});}, 128); } } );
